...
def test_energy():
    ...
    initial_energy = dpf.calculate_total_energy(initial_positions, l1, l2, m1, m2)

    position_list = dpf.integrate(initial_positions, l1, l2, m1, m2, t_max, dt)
    final_energy = dpf.calculate_total_energy(position_list[-1][1], l1, l2, m1, m2)
...
def test_cartesian_coordinates():
    ...
    actual_result = dpf.map_to_cartesian_coordinates(0, 0, 1, 1)
    assert np.allclose(expected_result, actual_result)

    expected_result = np.array([1, 0, 2, 0])
    actual_result = dpf.map_to_cartesian_coordinates(1 / 2 * np.pi, 1 / 2 * np.pi, 1, 1)
    assert np.allclose(expected_result, actual_result)

    expected_result = np.array([0, -1, 0, -2])
    actual_result = dpf.map_to_cartesian_coordinates(np.pi, np.pi, 1, 1)
    assert np.allclose(expected_result, actual_result)

    expected_result = np.array([-1, 0, -2, 0])
    actual_result = dpf.map_to_cartesian_coordinates(3 / 2 * np.pi, 3 / 2 * np.pi, 1, 1)
    assert np.allclose(expected_result, actual_result)

    expected_result = np.array([0, 1, 0, 2])
    actual_result = dpf.map_to_cartesian_coordinates(2 * np.pi, 2 * np.pi, 1, 1)
    assert np.allclose(expected_result, actual_result)
